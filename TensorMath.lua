...
interface:print('')
interface:print('#include "THZCTensorMath.h"')
interface:print('#include "THZC.h"')
interface:print('#include "luaT.h"')
interface:print('#include "torch/utils.h"')
interface:print('#include "cuComplex.h"')
interface:print('#include "complex.h"')
interface:print('')
interface:print('')
...
local typename1 = 'CudaTensor'
wrap.types.CudaTensor = {

   helpname = function(arg)
      if arg.dim then
         return string.format('%s~%dD', typename1, arg.dim)
      else
         return typename1
      end
   end,
   ...
   declare = function(arg)
      local txt = {}
      table.insert(txt, string.format("TH%s *arg%d = NULL;", typename1, arg.i))
   ...
   check = function(arg, idx)
      if arg.dim then
         return string.format('(arg%d = luaT_toudata(L, %d, "torch.%s")) && (arg%d->nDimension == %d)', arg.i, idx, typename1, arg.i, arg.dim)
      else
         return string.format('(arg%d = luaT_toudata(L, %d, "torch.%s"))', arg.i, idx, typename1)
      end
   end,
   ...
   init = function(arg)
      if type(arg.default) == 'boolean' then
         return string.format('arg%d = TH%s_new(cutorch_getstate(L));', arg.i, typename1)
   ...
   precall = function(arg)
      ...
      if arg.default and arg.returned then
         ...
         if type(arg.default) == 'boolean' then -- boolean: we did a new()
            table.insert(txt, string.format('luaT_pushudata(L, arg%d, "torch.%s");', arg.i, typename1))
         else  -- otherwise: point on default tensor --> retain
            table.insert(txt, string.format('{'))
            table.insert(txt, string.format('TH%s_retain(arg%d);', typename1, arg.i)) -- so we need a retain
            table.insert(txt, string.format('luaT_pushudata(L, arg%d, "torch.%s");', arg.i, typename1))
            table.insert(txt, string.format('}'))
         end
...
local typename = 'ZCudaTensor'
wrap.types.ZCudaTensor = {
...
wrap.types.complex = {
  ...
  declare = function(arg)
               ...
               return string.format("float complex arg%d = 0;", arg.i)
            end,
  ...
  read = function(arg, idx)
            return string.format("arg%d = (float complex)lua_tonumber(L, %d);", arg.i, idx)
         end,

  init = function(arg)
            -- otherwise do it here
            return string.format("arg%d = %s;", arg.i, 0)
         end,
...
function interface.luaname2wrapname(self, name)
   return string.format('zcutorch_ZCudaTensor_%s', name)
end

local function cname(name)
   return string.format('THZCudaTensor_%s', name)
end

local function lastdim(argn)
   return function(arg)
      return string.format("THZCudaTensor_nDimension(cutorch_getstate(L), %s)", arg.args[argn]:carg())
   end
end

local function lastdimarray(argn)
   return function(arg)
      return string.format("THZCudaTensor_nDimension(cutorch_getstate(L), arg%d_data[0])", arg.args[argn].i)
   end
end
...
local Tensor = "ZCudaTensor"
local CTensor = "CudaTensor"
local real = "complex"
local fl = "float"
print("----------------------------------- 1")
wrap("zero",
...
wrap("squeeze",
     ...
     {{name=Tensor, default=true, returned=true, postcall=function(arg)
          ...
          if arg.returned then
             ...
             table.insert(txt, string.format('lua_pushnumber(L, (lua_Number)(THZCudaTensor_get1d(cutorch_getstate(L), arg%d, 0)));', arg.i))
          end
          return table.concat(txt, '\n')
     end},
      {name=Tensor}},
...
method:register("m_zcutorch_ZCudaTensorMath__")
interface:print(method:tostring())
method:clearhistory()
interface:register("zcutorch_ZCudaTensorMath__")
