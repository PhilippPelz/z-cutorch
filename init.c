...
int createSingleDeviceEvents(lua_State *L, THCState *state, int arg, int device,
                             cudaEvent_t *event) {
...
void waitSingleDeviceEvents(lua_State *L, THCState *state, int arg, int device,
                            cudaEvent_t *event, int numEvents) {
  /* Push table to top */
  lua_pushvalue(L, arg);
  ...
  while (lua_next(L, -2)) {
    int streamId = (int)lua_tonumber(L, -1);
    cudaStream_t stream = THCState_getDeviceStream(state, device, streamId);
...
static int cutorch_setDevice(lua_State *L) {
  ...
  THCState_setBlasHandle(state, device,
                         THCState_getCurrentBlasHandleIndex(state));
...
static int cutorch_getDeviceProperties(lua_State *L) {
  ...
  // switch context to given device so the call to cudaMemGetInfo is for the
  // correct device
  int oldDevice;
