...
void THZCudaTensor_copyZFloat(THCState *state, THZCudaTensor *self, struct THZFloatTensor *src)
{
  THArgCheck(THZCudaTensor_nElement(state, self) == THZFloatTensor_nElement(src), 2, "sizes do not match");
...
{
  THArgCheck(THZFloatTensor_nElement(self) == THZCudaTensor_nElement(state, src), 2, "sizes do not match");
...
void THZCudaTensor_copyAsyncZFloat(THCState *state, THZCudaTensor *self, struct THZFloatTensor *src)
{
  THArgCheck(THZCudaTensor_nElement(state, self) == THZFloatTensor_nElement(src), 2, "sizes do not match");
  THArgCheck(THZCudaTensor_isContiguous(state, self), 2, "Target tensor must be contiguous");
  THArgCheck(THZFloatTensor_isContiguous(src), 3, "Source tensor must be contiguous");
...
void THZFloatTensor_copyAsyncZCuda(THCState *state, THZFloatTensor *self, struct THZCudaTensor *src)
{
  THArgCheck(THZFloatTensor_nElement(self) == THZCudaTensor_nElement(state, src), 2, "sizes do not match");
  THArgCheck(THZFloatTensor_isContiguous(self), 2, "Target tensor must be contiguous");
  THArgCheck(THZCudaTensor_isContiguous(state, src), 3, "Source tensor must be contiguous");

  if (THZFloatTensor_nElement(self) == 0) return;
