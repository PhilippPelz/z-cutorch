...
__global__ void THZCudaTensor_kernel_indexFill(
   ccx *tensor, long* stride, float *index, long src_nDim,
   int dim, long idx_size, long tensor_size, long size_dim, ccx val
)
{
...
__global__ void THZCudaTensor_kernel_indexAdd(
   ccx *res, ccx *src, long* res_stride, float *index,
...
)
{
  ...
  if (thread_idx < flat_size)
  {
    ...
    for (int i=0; i<idx_size; i++)
    {
      ...
      atomicAdd((float*)(&res[resIdx + ((long)(index[i])-1)*res_stride[dim]]), src[targetIdx + i*res_stride[dim]].real());
      atomicAdd(((float*)(&res[resIdx + ((long)(index[i])-1)*res_stride[dim]])) + 1, src[targetIdx + i*res_stride[dim]].imag());
    }
  }
}
...
void THZCudaTensor_indexCopy(THCState *state, THZCudaTensor *res_, int dim, THCudaTensor *indices, THZCudaTensor *src)
{
  ...
  THZCudaTensor_kernel_indexCopy<<<nblocks, nthreads, 0, THCState_getCurrentStream(state)>>>(
    (ccx*)THZCudaTensor_data(state, res_), (ccx*)THZCudaTensor_data(state, src),
...
void THZCudaTensor_indexAdd(THCState *state, THZCudaTensor *res_, int dim, THCudaTensor *indices, THZCudaTensor *src)
{
  THAssert(THZCudaTensor_checkGPU(state, 2, res_, src));
  long *stride_;
  long nIndex = indices->size[0];
...
void THZCudaTensor_indexFillf(THCState *state, THZCudaTensor *res_, int dim, THCudaTensor *indices, float val)
{
  ...
  THZCudaTensor_kernel_indexFill<<<nblocks, nthreads, 0, THCState_getCurrentStream(state)>>>(
    (ccx*)THZCudaTensor_data(state, res_), stride_, THCudaTensor_data(state, indices),
    res_->nDimension, dim, nIndex, nRes, res_->size[dim], ccx(val,0)
  );
...
void THZCudaTensor_indexFill(THCState *state, THZCudaTensor *res_, int dim, THCudaTensor *indices, cx val)
{
  ...
  THZCudaTensor_kernel_indexFill<<<nblocks, nthreads, 0, THCState_getCurrentStream(state)>>>(
    (ccx*)THZCudaTensor_data(state, res_), stride_, THCudaTensor_data(state, indices),
    res_->nDimension, dim, nIndex, nRes, res_->size[dim], toCcx(val)
  );
...
__global__ void THZCudaTensor_kernel_indexSelect_contiguous(
  ccx *tensor, ccx *src, long stride, float *index, long idxSize)
{
...
__global__ void THZCudaTensor_kernel_indexSelect(
  ZTensorInfo<unsigned long> dest,
  ZTensorInfo<unsigned long> src,
...
)
{
  for (unsigned long linearIndex = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    unsigned long destOffset =
      ZIndexToOffset<unsigned long, -1>::get(linearIndex, dest);
    ...
    unsigned long srcOffset =
      ZIndexToOffset<unsigned long, -1>::get(srcIndex, src);
...
void THZCudaTensor_indexSelect(THCState *state, THZCudaTensor *res, THZCudaTensor *src, int dim, THCudaTensor *indices)
{
      ...
      dim == 0)
  {
    ...
    THZCudaTensor_kernel_indexSelect_contiguous<<<nblocks, nthreads, 0, stream>>>(
      (ccx*)THZCudaTensor_data(state, res),
      (ccx*)THZCudaTensor_data(state, src),
      stride,
  ...
  ZTensorInfo<unsigned long> destInfo(state, res, NoCollapseDims);
