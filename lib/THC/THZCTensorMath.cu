...
struct TensorFillReOp {
  TensorFillReOp(float v) : val(v) {}
  __device__ __forceinline__ void operator()(cx* v) { *v = make_cuComplex(val,v.y); }

  const float val;
};
struct TensorFillImOp {
  TensorFillImOp(float v) : val(v) {}
  __device__ __forceinline__ void operator()(cx* v) { *v = make_cuComplex(v.x,val); }
