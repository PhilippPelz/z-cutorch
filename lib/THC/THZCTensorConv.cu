...
//   __global__ void conv2generic(ccx *input, ccx *kernel, ccx *output,
...
//               ccx *input_p = (ccx*)(input + ii*input_h*input_w + yy*stride_h*input_w + xx*stride_w);
//               ccx *output_p = (ccx*)(output + oo*output_h*output_w + yy*output_w + xx);
//               ccx *kernel_p = (ccx*)(shared_kernel + (ii % input_n)*kernel_w*kernel_h + koffset);
//               ccx sum = 0;
//               if (swapkernel) {
// #pragma unroll
...
//             ccx *input_p = (ccx*)(input + ii*input_h*input_w + yy*stride_h*input_w + xx*stride_w);
//             ccx *output_p = (ccx*)(output + oo*output_h*output_w + yy*output_w + xx);
//             ccx *kernel_p = (ccx*)(kernel + ((oo % output_n) * input_n + (ii % input_n))*kernel_w*kernel_h + koffset);
//             ccx sum = 0;
//             if (swapkernel) {
//               for(ky = 0; ky < kernel_h; ky++) {
// #pragma unroll 5
 ...
 *   - the swapkernel flag can be used to generate a conv2 instead of xcorr2
 */
// __global__ void conv2genericrev(ccx *input, ccx *kernel, ccx *output,
//                                 int input_n, int input_h, int input_w,
//                                 int kernel_n, int kernel_h, int kernel_w,
...
//   __shared__ ccx shared_output[CUDA_SHARED_MEM_SIZE];
//
//   // generate tid outputs in shared memory
//   ccx *output_s = shared_output + tid*output_w*output_h;
//
//   // convolution loop
//   int xx, yy, kx, ky;
//   yy = threadIdx.y;
//   ccx *output_p = output_s + yy * output_w;
//   for(xx=0; xx<output_w; xx++) {
//     // Dot product in two dimensions... (between input image and kernel)
//     ccx *input_p = (ccx*)(input + (ii + batch*input_n)*input_h*input_w + yy*stride_h*input_w + xx*stride_w);
//     ccx *kernel_p = (ccx*)(kernel + (kk + batch*kernel_n)*kernel_w*kernel_h);
//     ccx sum = 0;
//     for(ky=0; ky<kernel_h; ky++) {
 ...
 */
// THZC_API void THZCudaTensor_conv2Dmv(THCState *state, THZCudaTensor *output, float beta, THZCudaTensor *input,
//                                    THZCudaTensor *kernel, long srow, long scol, const char *type)
// {
//   THAssert(THZCudaTensor_checkGPU(state, 3, output, input, kernel));
...
//   kernel = THZCudaTensor_newContiguous(state, kernel);
//
...
//
//     // use temp buffer
//     THZCudaTensor *inputP = THZCudaTensor_new(state);
//
...
//     THZCudaTensor_resize3d(state, inputP, nInputPlane, nInputRowsPadded, nInputColsPadded);
//     THZCudaTensor_zero(state, inputP);
//
//     THZCudaTensor *centered = THZCudaTensor_new(state);
//     THZCudaTensor_narrow(state, centered, inputP, 2, nKernelCols-1, nInputCols);
//     THZCudaTensor_narrow(state, centered, NULL, 1, nKernelRows-1, nInputRows);
//     THZCudaTensor_copy(state, centered, input);
//     THZCudaTensor_free(state, centered);
//
...
//
//   if (beta == 0 || nelem != THZCudaTensor_nElement(state, output)) {
//     THZCudaTensor_zero(state, output);
//   } else if (beta != 1) {
//     THZCudaTensor_mul(state, output, output, beta);
//   }
//
//   ccx *input_data = (ccx*)THZCudaTensor_data(state, input);
//   ccx *weight_data = (ccx*)THZCudaTensor_data(state, kernel);
//   ccx *output_data = (ccx*)THZCudaTensor_data(state, output);
//
//   // cuda blocks & threads:
...
//
//   // clean
//   if (*type != 'f') THZCudaTensor_free(state, input);
//   THZCudaTensor_free(state, kernel);
//
//   // check for errors
 ...
 * matrix vector product like: y <- Ax + beta*y
 */
// THZC_API void THZCudaTensor_conv2Dmm(THCState *state, THZCudaTensor *output, float beta, THZCudaTensor *input,
//                                    THZCudaTensor *kernel, long srow, long scol, const char *type)
// {
//   THAssert(THZCudaTensor_checkGPU(state, 3, output, input, kernel));
...
//     long nInputRowsPadded = (nOutputRows - 1) * srow + nKernelRows;
//     long nInputColsPadded = (nOutputCols - 1) * scol + nKernelCols;
//     THZCudaTensor_resize4d(state, inputP, nbatch, nInputPlane, nInputRowsPadded, nInputColsPadded);
//     THZCudaTensor_zero(state, inputP);
//
//     THZCudaTensor *centered = THZCudaTensor_new(state);
//     THZCudaTensor_narrow(state, centered, inputP, 3, nKernelCols-1, nInputCols);
//     THZCudaTensor_narrow(state, centered, NULL, 2, nKernelRows-1, nInputRows);
//     THZCudaTensor_copy(state, centered, input);
//     THZCudaTensor_free(state, centered);
//
//     // remap input to newly created tensor
...
//
//   long nelem = THZCudaTensor_nElement(state, output);
//   THZCudaTensor_resize4d(state, output, nbatch, nOutputPlane, nOutputRows, nOutputCols);
//
//   if (beta == 0 || nelem != THZCudaTensor_nElement(state, output)) {
//     THZCudaTensor_zero(state, output);
//   } else if (beta != 1) {
 ...
 */
// THZC_API void THZCudaTensor_conv2DRevger(THCState *state, THZCudaTensor *output, float beta, float alpha,
//                                        THZCudaTensor *input, THZCudaTensor *kernel,
//                                        long srow, long scol)
// {
//   THAssert(THZCudaTensor_checkGPU(state, 3, output, input, kernel));
//   long nInputPlane, nInputRows, nInputCols;
...
//
//   long nelem = THZCudaTensor_nElement(state, output);
//   THZCudaTensor_resize4d(state, output, nKernelPlane, nInputPlane, nOutputRows, nOutputCols);
//
//   if (nelem == 0 || beta == 0 || nelem != THZCudaTensor_nElement(state, output)) {
//     THZCudaTensor_zero(state, output);
//   } else if (beta != 1) {
//     THZCudaTensor_mul(state, output, output, beta);
//   }
//
//   ccx *input_data = (ccx*)THZCudaTensor_data(state, input);
//   ccx *kernel_data = (ccx*)THZCudaTensor_data(state, kernel);
//   ccx *output_data = (ccx*)THZCudaTensor_data(state, output);
//
 ...
 */
// THZC_API void THZCudaTensor_conv2DRevgerm(THCState *state, THZCudaTensor *output, float beta, float alpha,
//                                         THZCudaTensor *input, THZCudaTensor *kernel,
//                                         long srow, long scol)
// {
//   long nInputPlane, nInputRows, nInputCols;
 ...
 */
// template <bool swapkernel, int T_kernel_h, int T_kernel_w>
//   __global__ void conv2mapgeneric(ccx *input, ccx *kernel, ccx *output,
//                                   int input_n, int input_h, int input_w,
//                                   int kernel_n, int kernel_h, int kernel_w,
 ...
 * matrix vector product like: y <- Ax + beta*y
 */
// THZC_API void THZCudaTensor_conv2Dmap(THCState *state, THZCudaTensor *output, THZCudaTensor *input,
//                                     THZCudaTensor *kernel, long stride_x, long stride_y,
//                                     THCudaTensor *table, long fanin)
// {
//   THAssert(THZCudaTensor_checkGPU(state, 4, output, input, kernel, table));
//   long nInputPlane, nInputRows, nInputCols;
//   long nKernelRows, nKernelCols;
//   long nOutputPlane, nOutputRows, nOutputCols;
//
