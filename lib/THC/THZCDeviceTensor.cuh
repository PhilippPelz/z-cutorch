#ifndef THZC_DEVICE_TENSOR_INC
#define THZC_DEVICE_TENSOR_INC
...
template <bool>
struct THZCStaticAssert;

template <>
struct THZCStaticAssert<true> {
};

#define thc_static_assert(expr) (THZCStaticAssert<(expr) != 0>())
...
class THZCDeviceTensor {
 public:
  ...
  typedef THZCDeviceTensor<T, Dim, IndexT, PtrTraits> TensorType;

  /// Default constructor
  __host__ __device__ THZCDeviceTensor();

  /// Constructor that calculates strides with no padding
  __host__ __device__ THZCDeviceTensor(DataPtrType data,
  ...
  isSameSizeAndStride(
    const THZCDeviceTensor<T, OtherDim, IndexT, PtrTraits>& rhs) const;

  /// Cast to a tensor of a different type of the same size and stride
  template <typename U>
  __host__ __device__ THZCDeviceTensor<U, Dim, IndexT, PtrTraits> cast();

  /// Const version of `cast`
  template <typename U>
  __host__ __device__
  const THZCDeviceTensor<U, Dim, IndexT, PtrTraits> cast() const;
  ...
  __host__ __device__ __forceinline__
  detail::THZCDeviceSubTensor<TensorType, Dim - 1, PtrTraits>
    operator[](IndexT);

  /// Returns a read/write view of a portion of our tensor (const).
  __host__ __device__ __forceinline__
  const detail::THZCDeviceSubTensor<TensorType, Dim - 1, PtrTraits>
  ...
  __host__ __device__ THZCDeviceTensor<T, Dim, IndexT, PtrTraits>
  ...
  template <int NewDim>
  __host__ __device__ THZCDeviceTensor<T, NewDim, IndexT, PtrTraits>
  upcastOuter();
  ...
  template <int NewDim>
  __host__ __device__
  THZCDeviceTensor<T, NewDim, IndexT, PtrTraits> downcastOuter();
  ...
  template <int NewDim>
  __host__ __device__
  THZCDeviceTensor<T, NewDim, IndexT, PtrTraits> downcastInner();

  /// Returns a tensor that is a view of the `SubDim`-dimensional slice
  /// of this tensor, starting at `at`.
  template <int SubDim>
  __host__ __device__ THZCDeviceTensor<T, SubDim, IndexT, PtrTraits>
  view(DataPtrType at);
...
class THZCDeviceSubTensor<TensorType, 0, PtrTraits> {
 public:
  __host__ __device__ THZCDeviceSubTensor<TensorType, 0, PtrTraits>
  ...
  friend class THZCDeviceSubTensor<TensorType, 1, PtrTraits>;

  /// Our parent tensor can create us
  friend class THZCDeviceTensor<typename TensorType::DataType,
                               1,
                               typename TensorType::IndexType,
                               PtrTraits>;

  __host__ __device__ __forceinline__ THZCDeviceSubTensor(
    TensorType& t,
...
/// A `SubDim`-rank slice of a parent THZCDeviceTensor
template <typename TensorType,
          int SubDim,
          template <typename U> class PtrTraits>
class THZCDeviceSubTensor {
 public:
  ...
  /// `SubDim` - 1 tensor).
  __host__ __device__ __forceinline__
  THZCDeviceSubTensor<TensorType, SubDim - 1, PtrTraits>
    operator[](typename TensorType::IndexType index) {
    return THZCDeviceSubTensor<TensorType, SubDim - 1, PtrTraits>(
      tensor_,
  ...
  __host__ __device__ __forceinline__
  const THZCDeviceSubTensor<TensorType, SubDim - 1, PtrTraits>
  ...
  /// Returns a tensor that is a view of the SubDim-dimensional slice
  /// of this tensor, starting where our data begins
  THZCDeviceTensor<typename TensorType::DataType,
               SubDim,
               typename TensorType::IndexType,
               PtrTraits> view() {
 ...
 private:
  /// One dimension greater can create us
  friend class THZCDeviceSubTensor<TensorType, SubDim + 1, PtrTraits>;
...
__host__ __device__ __forceinline__
detail::THZCDeviceSubTensor<THZCDeviceTensor<T, Dim, IndexT, PtrTraits>,
                        Dim - 1, PtrTraits>
THZCDeviceTensor<T, Dim, IndexT, PtrTraits>::operator[](IndexT index) {
  return detail::THZCDeviceSubTensor<TensorType, Dim - 1, PtrTraits>(
    detail::THZCDeviceSubTensor<TensorType, Dim, PtrTraits>(
      *this, data_)[index]);
}
...
template <typename T, int Dim,
          typename IndexT, template <typename U> class PtrTraits>
__host__ __device__ __forceinline__
const detail::THZCDeviceSubTensor<THZCDeviceTensor<T, Dim, IndexT, PtrTraits>,
                              Dim - 1, PtrTraits>
THZCDeviceTensor<T, Dim, IndexT, PtrTraits>::operator[](IndexT index) const {
  return detail::THZCDeviceSubTensor<TensorType, Dim - 1, PtrTraits>(
    detail::THZCDeviceSubTensor<TensorType, Dim, PtrTraits>(
      const_cast<TensorType&>(*this), data_)[index]);
}

#include "THZCDeviceTensor-inl.cuh"

#endif // THZC_DEVICE_TENSOR_INC
