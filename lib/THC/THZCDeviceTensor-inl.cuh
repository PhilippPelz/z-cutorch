...
__host__ __device__
THZCDeviceTensor<T, Dim, IndexT, PtrTraits>::THZCDeviceTensor()
    : data_(NULL) {
...
THZCDeviceTensor(DataPtrType data, const IndexT sizes[Dim])
    : data_(data) {
  thc_static_assert(Dim > 0);
...
THZCDeviceTensor<T, Dim, IndexT, PtrTraits>::isSameSizeAndStride(
...
template <typename T, int Dim,
          typename IndexT, template <typename U> class PtrTraits>
template <typename U>
__host__ __device__ THZCDeviceTensor<U, Dim, IndexT, PtrTraits>
THZCDeviceTensor<T, Dim, IndexT, PtrTraits>::cast() {
  thc_static_assert(sizeof(U) == sizeof(T));

  return THZCDeviceTensor<U, Dim, IndexT, PtrTraits>(
...
template <typename T, int Dim,
          typename IndexT, template <typename U> class PtrTraits>
template <typename U>
__host__ __device__ const THZCDeviceTensor<U, Dim, IndexT, PtrTraits>
THZCDeviceTensor<T, Dim, IndexT, PtrTraits>::cast() const {
  thc_static_assert(sizeof(U) == sizeof(T));
...
THZCDeviceTensor<T, Dim, IndexT, PtrTraits>::numElements() const {
  long size = getSize(0);
...
template <typename T, int Dim,
          typename IndexT, template <typename U> class PtrTraits>
__host__ __device__ bool
THZCDeviceTensor<T, Dim, IndexT, PtrTraits>::isContiguous() const {
  long prevSize = 1;
...
template <typename T, int Dim,
          typename IndexT, template <typename U> class PtrTraits>
__host__ __device__ bool
THZCDeviceTensor<T, Dim, IndexT, PtrTraits>::isConsistentlySized(int i) const {
...
template <typename T, int Dim,
          typename IndexT, template <typename U> class PtrTraits>
__host__ __device__ bool
THZCDeviceTensor<T, Dim, IndexT, PtrTraits>::isConsistentlySized() const {
  for (int i = 0; i < Dim; ++i) {
...
template <typename T, int Dim,
          typename IndexT, template <typename U> class PtrTraits>
__host__ __device__ bool
THZCDeviceTensor<T, Dim, IndexT, PtrTraits>::isContiguousDim(int i) const {
...
THZCDeviceTensor<T, Dim, IndexT, PtrTraits>::transpose(int dim1,
                                                      int dim2) const {
#ifdef __CUDA_ARCH__
  // Device code
  ...
  return THZCDeviceTensor<T, Dim, IndexT, PtrTraits>(data_, newSize, newStride);
}
...
THZCDeviceTensor<T, Dim, IndexT, PtrTraits>::upcastOuter() {
  ...
  return THZCDeviceTensor<T, NewDim, IndexT, PtrTraits>(
...
template <int NewDim>
__host__ __device__ THZCDeviceTensor<T, NewDim, IndexT, PtrTraits>
THZCDeviceTensor<T, Dim, IndexT, PtrTraits>::upcastInner() {
  // Can only create tensors of greater dimension
  thc_static_assert(NewDim > Dim);
...
template <int NewDim>
__host__ __device__ THZCDeviceTensor<T, NewDim, IndexT, PtrTraits>
THZCDeviceTensor<T, Dim, IndexT, PtrTraits>::downcastOuter() {
...
template <int NewDim>
__host__ __device__ THZCDeviceTensor<T, NewDim, IndexT, PtrTraits>
THZCDeviceTensor<T, Dim, IndexT, PtrTraits>::downcastInner() {
  // Can only create tensors of lesser dimension
  thc_static_assert(NewDim < Dim);
...
THZCDeviceTensor<T, Dim, IndexT, PtrTraits>::view(DataPtrType at) {
  ...
  return THZCDeviceTensor<T, SubDim, IndexT, PtrTraits>(
...
template <int SubDim>
__host__ __device__ THZCDeviceTensor<T, SubDim, IndexT, PtrTraits>
THZCDeviceTensor<T, Dim, IndexT, PtrTraits>::view() {
...
template <typename T, int Dim,
          typename IndexT, template <typename U> class PtrTraits>
void
THZCDeviceTensor<T, Dim, IndexT, PtrTraits>::zero(cudaStream_t stream) {
#ifdef __CUDA_ARCH__
  assert(isContiguous());
#else
  if (!isContiguous()) {
