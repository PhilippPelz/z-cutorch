...
void THZCCheckTensorDims(THCState* state, THZCudaTensor* tensor, int arg) {
  long dims = THZCudaTensor_nDimension(state, tensor);
  THArgCheck(dims <= MAX_CUTORCH_DIMS, arg, CUTORCH_DIM_WARNING);
}

bool THZC_canUse32BitIndexMath(THCState* state, THZCudaTensor* t) {
  long elements = THZCudaTensor_nElement(state, t);
  ...
  for (int i = THZCudaTensor_nDimension(state, t) - 1; i >= 0; --i) {
    long curDimIndex = linearId % THZCudaTensor_size(state, t, i);
    long curDimOffset = curDimIndex * THZCudaTensor_stride(state, t, i);
    offset += curDimOffset;
    linearId /= THZCudaTensor_size(state, t, i);
  }
...
bool THZC_getGridFromTiles(long gridTiles, dim3& grid) {
  ...
  if (gridTiles > MAX_GRID_SIZE) {
    gridTiles = THZCCeilDiv(gridTiles, MAX_GRID_SIZE);
...
bool THZC_overlappingIndices(THCState* state, THZCudaTensor* t) {
  ...
  int dims = THZCudaTensor_nDimension(state, t);
  int nonSize1Dims = 0;
  for (int i = 0; i < dims; ++i) {
    long size = THZCudaTensor_size(state, t, i);
    if (size > 1) {
      info[nonSize1Dims].size = size;
      info[nonSize1Dims].stride = THZCudaTensor_stride(state, t, i);
      ++nonSize1Dims;
    }
  }
