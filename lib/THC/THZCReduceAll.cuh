#ifndef THZC_REDUCEALL_INC
#define THZC_REDUCEALL_INC
...
#define THZC_REDUCE_ALL_BLOCK_SIZE 1024L

// Cutoff size for two-pass reduction
#define THZC_TWO_PASS_REDUCTION_SIZE 2048L
...
__global__ void
THZCudaTensor_reduceAll(ZTensorInfo<IndexType> in,
  ...
  for (IndexType i = threadIdx.x; i < totalElements; i += blockDim.x) {
    const IndexType inOffset = ZIndexToOffset<IndexType, ADims>::get(i, in);
...
template <typename ModifyOp, typename ReduceOp, typename IndexType, int ADims>
__global__ void
THZCudaTensor_reduceAllf(ZTensorInfo<IndexType> in,
                       IndexType totalElements,
                       float init,
                       ModifyOp modifyOp,
                       ReduceOp reduceOp,
  ...
  r = reduceBlock<float, ReduceOp>(smemf, blockDim.x, r, reduceOp, init);
...
__device__ __forceinline__ IndexType getStartIndex(IndexType totalSize) {
  IndexType sizePerBlock = THZCCeilDiv(totalSize, (IndexType) gridDim.x);
...
template <typename ModifyOp, typename ReduceOp, typename IndexType, int ADims>
__global__ void
THZCudaTensor_reduceAllPass1(ZTensorInfo<IndexType> in,
                            IndexType totalElements,
                            ccx init,
                            ModifyOp modifyOp,
                            ReduceOp reduceOp,
  ...
  // Reduce within the block
  extern __shared__ ccx smem[];
  r = reduceBlock<ccx, ReduceOp>(smem, (int)blockDim.x, r, reduceOp, init);
...
template <typename ModifyOp, typename ReduceOp, typename IndexType, int ADims>
__global__ void
THZCudaTensor_reduceAllPass1f(ZTensorInfo<IndexType> in,
                            IndexType totalElements,
                            float init,
                            ModifyOp modifyOp,
                            ReduceOp reduceOp,
...
__global__ void
THZCudaTensor_reduceAllPass2(int numPass1Blocks,
                            ccx init,
                            ReduceOp reduceOp,
                            ccx* scratchSpace,
                            ccx* out) {
  ...
  // Reduce within the block
  extern __shared__ ccx smem[];
  r = reduceBlock<ccx, ReduceOp>(smem, numPass1Blocks, r, reduceOp, init);
...
template <typename ReduceOp, typename IndexType>
__global__ void
THZCudaTensor_reduceAllPass2f(int numPass1Blocks,
                            float init,
                            ReduceOp reduceOp,
                            float* scratchSpace,
                            float* out) {
  ...
  // Reduce within the block
  extern __shared__ float smemf[];
  r = reduceBlock<float, ReduceOp>(smemf, numPass1Blocks, r, reduceOp, init);
...
inline bool isTwoPassReductionSize(long elements) {
  return (elements > THZC_TWO_PASS_REDUCTION_SIZE);
}

inline long getTwoPassBlocks(THCState* state, long elements) {
  long numBlocks = THZCCeilDiv(elements, THZC_REDUCE_ALL_BLOCK_SIZE);

  // We can only have as many blocks as there is scratch space
  long scratchSpace = THCState_getCurrentDeviceScratchSpaceSize(state) / sizeof(ccx);
...
inline void getPass1ReduceBlockGrid(THCState* state, long elements,
  ...
  block = dim3(THZC_REDUCE_ALL_BLOCK_SIZE);
}
...
void callReduceAll(THCState* state,
                   const ZTensorInfo<IndexType>& in,
                   long totalElements,
                   ccx init,
  ...
  if (isTwoPassReductionSize(totalElements)) {
    ...
    size_t smemSize = block.x * sizeof(ccx);

    THZCudaTensor_reduceAllPass1<ModifyOp, ReduceOp, IndexType, ADims><<<grid, block, smemSize, THCState_getCurrentStream(state)>>>(in, (IndexType) totalElements, init, modifyOp, reduceOp,(ccx*) THCState_getCurrentDeviceScratchSpace(state));

    int numPass1Blocks = grid.x;
    getPass2ReduceBlockGrid(state, totalElements, grid, block);
    smemSize = block.x * sizeof(ccx);

    THZCudaTensor_reduceAllPass2<ReduceOp, IndexType><<<grid, block, smemSize, THCState_getCurrentStream(state)>>>(numPass1Blocks, init, reduceOp,(ccx*) THCState_getCurrentDeviceScratchSpace(state),devOut);

  } else {
    getSinglePassReduceBlockGrid(totalElements, grid, block);
    size_t smemSize = block.x * sizeof(ccx);

    THZCudaTensor_reduceAll<ModifyOp, ReduceOp, IndexType, ADims><<<grid, block, smemSize, THCState_getCurrentStream(state)>>>(in, (IndexType) totalElements, init, modifyOp, reduceOp, devOut);
  }
}

template <typename ModifyOp, typename ReduceOp, typename IndexType, int ADims>
void callReduceAllf(THCState* state,
                   const ZTensorInfo<IndexType>& in,
                   long totalElements,
                   float init,
                   const ModifyOp& modifyOp,
  ...
  if (isTwoPassReductionSize(totalElements)) {
    ...
    THZCudaTensor_reduceAllPass1f<ModifyOp, ReduceOp, IndexType, ADims>
    ...
    THZCudaTensor_reduceAllPass2f<ReduceOp, IndexType>
      <<<grid, block, smemSize, THCState_getCurrentStream(state)>>>(
    ...
    THZCudaTensor_reduceAllf<ModifyOp, ReduceOp, IndexType, ADims>
      <<<grid, block, smemSize, THCState_getCurrentStream(state)>>>(
...
bool THZCudaTensor_reduceAll(THCState* state,
                            THZCudaTensor* in,
                            const ModifyOp& modifyOp,
                            const ReduceOp& reduceOp,
                            ccx init,
  ...
  if (THZCudaTensor_nDimension(state, in) > MAX_CUTORCH_DIMS) {
    return false;
  }
  ...
  if (!outOnDevice) {
    ...
    devOut = (ccx*) THCState_getCurrentDeviceScratchSpace(state);
  }
...
#define HANDLE_IN_CASE(TYPE, IN)                    \
  ...
  if (THZC_canUse32BitIndexMath(state, in)) {
    ZTensorInfo<unsigned int> inInfo(state, in);

    HANDLE_IN_CASE(unsigned int, inInfo.dims);
  } else {
    ZTensorInfo<unsigned long long> inInfo(state, in);
...
#undef HANDLE_IN_CASE
  ...
  if (!outOnDevice) {
    cudaMemcpy(out, devOut, sizeof(ccx), cudaMemcpyDeviceToHost);
  }
...
// to host-resident memory.
template <typename ModifyOp, typename ReduceOp>
bool THZCudaTensor_reduceAllf(THCState* state,
                            THZCudaTensor* in,
                            const ModifyOp& modifyOp,
                            const ReduceOp& reduceOp,
                            float init,
  ...
  // index can be similarly collapsed. That is what this unrolling is for.
#define HANDLE_CASE(TYPE, IN)                                           \
  callReduceAllf<ModifyOp, ReduceOp, TYPE, IN>(                          \
    state, inInfo, inElements, init, modifyOp, reduceOp, devOut);
...
#undef THZC_REDUCE_ALL_BLOCK_SIZE
#undef THZC_TWO_PASS_REDUCTION_SIZE

#endif // THZC_REDUCEALL_INC
