...
{
  THZCudaCheck(THZCudaMalloc(state, (void**)&gen->gen_states, MAX_NUM_BLOCKS * sizeof(curandStateMtgp32)));
  THZCudaCheck(THZCudaMalloc(state, (void**)&gen->kernel_params, sizeof(mtgp32_kernel_params)));
}
...
__host__ void destroyGenerator(THCState *state, Generator* gen)
{
  if (gen->gen_states)
  {
    THZCudaCheck(THZCudaFree(state, gen->gen_states));
    gen->gen_states = NULL;
  }
  if (gen->kernel_params)
  {
    THZCudaCheck(THZCudaFree(state, gen->kernel_params));
...
__host__ void THZCRandom_init(THCState* state, int devices, int current_device)
{
  ...
  // initialized on-demand in THZCRandom_setGenerator.
  initializeGenerator(state, rng_state->current_gen);
  THZCRandom_seed(state);
}

/* Destroy generators and free memory */
__host__ void THZCRandom_shutdown(THCState* state)
{
  THCRNGState* rng_state = state->rngState;
...
__host__ void THZCRandom_setGenerator(THCState* state, int device)
{
  THCRNGState* rng_state = state->rngState;
...
/* Random seed */
__host__ unsigned long THZCRandom_seed(THCState* state)
{
  unsigned long s = (unsigned long)time(0);
  THZCRandom_manualSeed(state, s);
  return s;
}

__host__ unsigned long THZCRandom_seedAll(THCState* state)
{
  unsigned long s = (unsigned long)time(0);
  THZCRandom_manualSeedAll(state, s);
  return s;
}

/* Manually set the seed */
__host__ void THZCRandom_manualSeed(THCState* state, unsigned long seed)
{
  THCRNGState* rng_state = state->rngState;
...
__host__ void THZCRandom_manualSeedAll(THCState* state, unsigned long seed)
{
  THCRNGState* rng_state = state->rngState;
  int currentDevice;
  ...
  for (int i = 0; i < rng_state->num_devices; ++i) {
    ...
    THZCRandom_setGenerator(state, i);
    THZCRandom_manualSeed(state, seed);
  }
  THZCudaCheck(cudaSetDevice(currentDevice));
  THZCRandom_setGenerator(state, currentDevice);
}

/* Get the initial seed */
__host__ unsigned long THZCRandom_initialSeed(THCState* state)
{
  return state->rngState->current_gen->initial_seed;
}

__host__ void THZCRandom_getRNGState(THCState* state, THByteTensor *rng_state)
{
  ...
  THZCudaCheck(cudaMemcpy(THByteTensor_data(rng_state), state->rngState->current_gen->gen_states,
...
__host__ void THZCRandom_setRNGState(THCState* state, THByteTensor *rng_state)
{
  if (state->rngState->current_gen == NULL)
  {
    THError("Random number generators have not been initialized.");
  ...
  THZCudaCheck(cudaMemcpy(state->rngState->current_gen->gen_states, THByteTensor_data(rng_state),
...
{                                                                              \
  ...
  int rounded_size = THZCCeilDiv(size, BLOCK_SIZE) * BLOCK_SIZE;                     \
...
#define NUM_BLOCKS min((int)THZCCeilDiv(size, (long) BLOCK_SIZE), MAX_NUM_BLOCKS)
THZC_API void THZCudaTensor_uniform(THCState* state, THZCudaTensor *self_, double a, double b)
{
  ...
  float *data = THZCudaTensor_data(state, self);
...
THZC_API void THZCudaTensor_bernoulli(THCState* state, THZCudaTensor *self_, double p)
{
  THAssert(THZCudaTensor_checkGPU(state, 1, self_));
  if (state->rngState->current_gen == NULL)
  {
...
THZC_API void THZCudaTensor_normal(THCState* state, THZCudaTensor *self_, double mean, double stdv)
{
  THAssert(THZCudaTensor_checkGPU(state, 1, self_));
  if (state->rngState->current_gen == NULL)
  {
...
THZC_API void THZCudaTensor_logNormal(THCState* state, THZCudaTensor *self_, double mean, double stdv)
{
  THAssert(THZCudaTensor_checkGPU(state, 1, self_));
  if (state->rngState->current_gen == NULL)
  {
...
THZC_API void THZCudaTensor_geometric(THCState* state, THZCudaTensor *self_, double p)
{
  THAssert(THZCudaTensor_checkGPU(state, 1, self_));
  if (state->rngState->current_gen == NULL)
  {
...
THZC_API void THZCudaTensor_exponential(THCState* state, THZCudaTensor *self_, double lambda)
{
  THAssert(THZCudaTensor_checkGPU(state, 1, self_));
  if (state->rngState->current_gen == NULL)
  {
...
THZC_API void THZCudaTensor_cauchy(THCState* state, THZCudaTensor *self_, double median, double sigma)
{
  THAssert(THZCudaTensor_checkGPU(state, 1, self_));
  if (state->rngState->current_gen == NULL)
  {
...
void THZCudaTensor_renormRows(struct THCState* state,
                             THZCudaTensor* t) {
  THAssert(THZCudaTensor_nDimension(state, t) == 2);
  long rows = THZCudaTensor_size(state, t, 0);
  long cols = THZCudaTensor_size(state, t, 1);
  ...
  renormRowsL1
    ...
    THCState_getCurrentStream(state)>>>(THZCudaTensor_data(state, t),
                                        rows, cols);
}
...
sampleMultinomialOnce(float* dest,
  ...
  for (long curDist = blockIdx.x;
    ...
    int chunks = THZCCeilDiv(categories, (int) blockDim.x);
...
THZC_API void THZCudaTensor_multinomial(struct THCState *state,
                                      THZCudaTensor *self,
                                      THZCudaTensor *prob_dist,
                                      int n_sample,
                                      int with_replacement)
{
  THAssert(THZCudaTensor_checkGPU(state, 2, self, prob_dist));
  if (state->rngState->current_gen == NULL) {
    THError("Random number generators have not been initialized.");
  }

  int inputSize = THZCudaTensor_nDimension(state, prob_dist);
  THArgCheck(inputSize > 0 && inputSize <= 2, 2,
             "prob_dist must be 1 or 2 dim");

  // Categories are in the innermost dimension
  long numDist =
    inputSize == 1 ? 1 : THZCudaTensor_size(state, prob_dist, 0);
  long numCategoriesLong =
    inputSize == 1 ? THZCudaTensor_size(state, prob_dist, 0) :
    THZCudaTensor_size(state, prob_dist, 1);
  ...
  THZCudaTensor* probDistContig =
    THZCudaTensor_newContiguous(state, prob_dist);

  // Restructure data for 2d
  if (inputSize == 1) {
    THZCudaTensor_resize2d(state, probDistContig, 1, numCategories);
  }

  THZCudaTensor_resize2d(state, self, numDist, n_sample);
  ...
  if (n_sample == 1) {
    ...
    THZCudaTensor_uniform(state, self, 0.0, 1.0);
    ...
    sampleMultinomialOnce
      ...
      THZCudaTensor_data(state, self),
      numDist,
      numCategories,
      THZCudaTensor_data(state, probDistContig));
  } else {
    // Generic, slow implementation with memory allocations

    // For sampling without replacement, we modify the distribution
    // for subsequent samples in this space
    THZCudaTensor* origDist = THZCudaTensor_new(state);
    THZCudaTensor_resizeAs(state, origDist, probDistContig);
    THZCudaTensor_copy(state, origDist, probDistContig);

    THZCudaTensor* normDist = THZCudaTensor_new(state);
    THZCudaTensor_resizeAs(state, normDist, probDistContig);

    THZCudaTensor* prefixSum = THZCudaTensor_new(state);

    // Renorm along rows
    THZCudaTensor_copy(state, normDist, origDist);
    THZCudaTensor_renormRows(state, normDist);

    // Prefix sum along rows
    THZCudaTensor_cumsum(state, prefixSum, normDist, 1);
    ...
    if (with_replacement) {
      ...
      sampleMultinomialWithReplacement
        <<<grid, block, 0, THCState_getCurrentStream(state)>>>(
          ...
          THZCudaTensor_data(state, prefixSum));
    } else {
      ...
      // distribution concurrently.
      long numBlocks = THZCCeilDiv(numDist, 4L);
      ...
      for (int sample = 0; sample < n_sample; ++sample) {
        ...
        sampleMultinomialWithoutReplacement
          <<<grid, block, 0, THCState_getCurrentStream(state)>>>(
            ...
            THZCudaTensor_data(state, self),
            numDist, numCategories,
            THZCudaTensor_data(state, origDist),
            THZCudaTensor_data(state, prefixSum));
      }
    }

    THZCudaTensor_free(state, prefixSum);
    THZCudaTensor_free(state, normDist);
    THZCudaTensor_free(state, origDist);
  }

  // Revert data restructuring based on input sizes
  if (inputSize == 1) {
    THZCudaTensor_resize1d(state, self, n_sample);
    ...
    THZCudaTensor_resize1d(state, probDistContig, numCategories);
  }

  THZCudaTensor_free(state, probDistContig);
}
