                                                   ...
                                                   BinaryFunction binary_op)
{
  ...
  unsigned row_size = THZCudaTensor_size(state, src, rdim);
                                     ...
                                     BinaryFunction binary_op)
{
  ...
  THZCudaTensor_resize(state, tgt1_, dim, NULL);
  THZCudaTensor_resize(state, tgt2_, dim, NULL);
  THLongStorage_free(dim);

  THZCudaTensor *tgt1 = THZCudaTensor_newContiguous(state, tgt1_);
  THZCudaTensor *tgt2 = THZCudaTensor_newContiguous(state, tgt2_);
  src = THZCudaTensor_newContiguous(state, src);

  if(dimension == THZCudaTensor_nDimension(state, src)-1) {
    THZCudaTensor_transformReduceInnermostDimIndex(state, tgt1, tgt2, src, init, binary_op);
  } else {
    THZCudaTensor_transformReduceOuterDimIndex(state, tgt1, tgt2, src, dimension, init, binary_op);
  }

  THZCudaTensor_free(state, src);
  THZCudaTensor_freeCopyTo(state, tgt1, tgt1_);
  THZCudaTensor_freeCopyTo(state, tgt2, tgt2_);
}
...
void THZCudaTensor_max(THCState *state, THZCudaTensor *values, THZCudaTensor *indices, THZCudaTensor *src, long dimension)
{
...
void THZCudaTensor_min(THCState *state, THZCudaTensor *values, THZCudaTensor *indices, THZCudaTensor *src, long dimension)
{
  THAssert(THZCudaTensor_checkGPU(state, 3, values, indices, src));
